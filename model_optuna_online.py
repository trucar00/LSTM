...
mu_sigma_path = Path(f"tuning/parameters_2024_optuna_online_new_rule_no_dist.pkl")
...
def cache_windows(mmsi_set, name, window, stride):
    out_path = Path(f"tuning/cache_{name}_w{window}_s{stride}_NEW_RULE_NO_DIST_online.pt")
    if out_path.exists():
...
def train_one_config(cfg, trial=None, max_epochs=6):
    ...
    train_cache = torch.load(f"tuning/cache_train_w{cfg['window']}_s{cfg['stride']}_NEW_RULE_NO_DIST_online.pt")
    val_cache   = torch.load(f"tuning/cache_val_w{cfg['window']}_s{cfg['stride']}_NEW_RULE_NO_DIST_online.pt")
...
study = optuna.create_study(
    direction="minimize",
    study_name="fishing_LSTM_search_NEW_RULE_NO_DIST",
    storage="sqlite:///tuning/optuna_LSTM_fishing_NEW_RULE_NO_DIST.db",   # so you can resume / inspect
    load_if_exists=True,
    pruner=optuna.pruners.MedianPruner(n_warmup_steps=2, n_startup_trials=5),
    sampler=optuna.samplers.TPESampler(seed=42),
)

study.optimize(objective, n_trials=37, show_progress_bar=False)
...
with open("tuning/best_params_LSTM_NEW_RULE_NO_DIST.json", "w") as f:
