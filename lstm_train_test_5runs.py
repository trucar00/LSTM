...
test_mmsi = all_mmsis_in(TEST_FILES)
print(f"Train (all 2023) vessels: {len(train_mmsi)} | Val (Q1, Q3 2024) vessels: {len(val_mmsi)} | Test (Q2, Q4 2024) vessels: {len(test_mmsi)}")
...
val_ds   = AISWindowDataset(VAL_FILES, val_mmsi, FEATURES, mu, sigma,
                            stride=STRIDE, window=WINDOW)
test_ds  = AISWindowDataset(TEST_FILES, test_mmsi, FEATURES, mu, sigma,
                            stride=STRIDE, window=WINDOW)
...
print(f"Preparing test set: {TEST_FILES}")
df_test = pd.concat(
    [pd.read_parquet(f) for f in TEST_FILES], ignore_index=True
)
...
results_csv_path = "multi_seeds_results/LSTM_temporal_train_2023_val_q1-q3_test_q2-q4_2024.csv"
